...
from MazeDriver.Control import ChallengeControl, PinControl, ScoreControl, AdminControl, mazeControl
from flask import render_template, request
from datetime import datetime
from werkzeug.utils import secure_filename
import os
from database import app, csrf
...
def studentChallenge():
    ...
    if request.method == "POST":
        ...
        if data:
            print(data)
            return render_template('VirtualMap.php', title='Student Challenge', data=data, map=data['grid'], year=datetime.now().year)
...
def studentSubmit():
    ...
    if request.method == "POST":
        ...
        map = query['map']
        solution = query['solution'].split(" ")
        maze = mazeControl.maze_control(map, solution)
        coords = maze.get_car()
        if coords:
            results = maze.move_car(coords[0],coords[1])
        if results:
            if results[2]:
                return render_template('GameOver.php')
            elif results[3]:
                return render_template('GameOver.php', error=results[3])
            else:
                return render_template('VirtualMap.php', update_map=results[0],solution=results[1])
